...
import { adminGetPromo } from "@/lib/admin/queries";
...
export default async function EditPromoPage({
  ...
  const [t, promo] = await Promise.all([
  ...
  const initial = {
    code: promo.code,
    discountType:
      (promo.discount_type === "fixed" ? "fixed" : "percentage") as "percentage" | "fixed",
