          ...
          <table className="min-w-full divide-y divide-border text-sm">
                ...
                return (
                  <tr key={i}>
                      ...
                      {hasDiscount ? (
                        <>
                          <div className="text-[10px] text-muted line-through">
                            {list.toFixed(2)} {proforma.currency}
                          </div>
                          <div className="font-semibold text-success">
                            {eff.toFixed(2)} {proforma.currency}
                          </div>
                        </>
                      ) : (
                        <>{list.toFixed(2)} {proforma.currency}</>
                    ...
                    <td className="px-4 py-2 text-right tabular-nums font-semibold">
                      {lineTotal.toFixed(2)} {proforma.currency}
                    </td>
                    <td className="px-4 py-2 text-right tabular-nums text-warning">
                      {cost != null
                        ? `${cost.toFixed(2)} ${proforma.currency}`
                        : "—"}
                    </td>
                    <td className="px-4 py-2 text-right tabular-nums">
                      {lineProfit != null && lineMarginPct != null ? (
                        <>
                          ...
                          >
                            {lineProfit >= 0 ? "+" : ""}
                            {lineProfit.toFixed(2)} {proforma.currency}
                          </div>
            ...
            <tfoot>
              {(() => {
                let before = 0;
                let after = 0;
                for (const it of proforma.items_snapshot) {
                  const qty = Number(it.quantity ?? 0);
                  const list = Number(it.unit_price ?? 0);
                  const dp =
                    it.discounted_unit_price != null ? Number(it.discounted_unit_price) : null;
                  ...
                  <>
                    <tr>
                      ...
                      <td className="px-4 py-2 text-right tabular-nums text-muted line-through">
                        {before.toFixed(2)} {proforma.currency}
                      </td>
              ...
              <tr className="bg-surface-elevated">
                ...
                {(() => {
                  // Roll up cost + profit for the admin-only footer
                  // cells. Lines without a cost don't contribute —
                  // shows "—" if even one item is uncosted.
                  let totalCost = 0;
                  let totalSell = 0;
                  ...
                  return (
                    <>
                      <td className="px-4 py-3 text-right tabular-nums text-warning">
                        {totalCost.toFixed(2)} {proforma.currency}
                      ...
                      <td className="px-4 py-3 text-right tabular-nums">
                        ...
                        >
                          {profit >= 0 ? "+" : ""}{profit.toFixed(2)} {proforma.currency}
                        </div>
                        <div className="text-[10px] text-muted">
                          {marginPct.toFixed(1)}%
                        </div>
