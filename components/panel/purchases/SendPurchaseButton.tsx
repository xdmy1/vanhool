...
/**
 * Per-row "Send to accountant" on /panel/achizitii (conta1 only). Sends
 * the entire purchase document — header + every line — in one email.
 * PIN-gated like every other accountant-facing button.
 */
export function SendPurchaseButton({ purchaseId }: { purchaseId: string }) {
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1 rounded-md border border-success bg-success/15 px-2 py-1 text-xs font-medium text-success transition-colors hover:bg-success/25"
        title={t("achizitii_row_send_accountant")}
      >
