-- Complete Orders and Payments Database Schema
...
CREATE POLICY "Users can view their own orders" ON orders
    FOR SELECT USING (
        auth.uid() = user_id OR 
        auth.uid() IN (SELECT id FROM auth.users WHERE email IN ('admin@autoparts.com'))
    );
...
CREATE POLICY "Admin can manage promocodes" ON promocodes
    FOR ALL USING (
        auth.uid() IN (SELECT id FROM auth.users WHERE email IN ('admin@autoparts.com'))
    );
...
-- Success message
SELECT 'Orders and Payments schema created successfully!' as status;